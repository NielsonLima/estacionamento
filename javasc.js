...
function renderGrid(){
  ...
  for(let i=0;i<TOTAL_SPOTS;i++){
    ...
    div.dataset.index = i;
    div.innerHTML = `<div class="num">Vaga ${i+1}</div>` + (spots[i] ? `<div class="meta">${spots[i].plate || ''}</div><div class="meta">${spots[i].name || ''}</div>` : '');
...
function openModal(index){
  ...
  // Atualizar displays de informações do veículo
  if(!data){
    // nova ocupação
    nameInput.value = '';
    ...
    vehicleTypeDisplay.textContent = (data.vehicleType === 'carro' ? 'Carro' : 'Moto') + (data.clientType === 'comerciario' ? ' (Comerciário)' : ' (Usuário)');
...
function openPrintWindow(html){
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Ticket</title><style>body{font-family:Arial;margin:0;padding:10px;display:flex;justify-content:center;align-items:center;height:100vh;background:#f0f0f0}@media print{body{background:#fff;height:auto}@page{size:auto;margin:0}}</style></head><body>${html}<div style="margin-top:15px;text-align:center"><button onclick="window.print();" style="padding:8px 16px;background:#2b7cff;color:#fff;border:none;border-radius:4px;cursor:pointer">Imprimir</button></div></body></html>`);
  w.document.close();
}
...
document.querySelectorAll('input[name="vehicleType"], input[name="clientType"]').forEach(input => {
  input.addEventListener('change', () => {
    ...
    vehicleTypeDisplay.textContent = (vehicleType === 'carro' ? 'Carro' : 'Moto') + (clientType === 'comerciario' ? ' (Comerciário)' : ' (Usuário)');
  });
});
