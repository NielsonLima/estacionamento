        ...
        function setupRealtimeListener() {
            ...
            const docRef = db.collection("parking").doc("spots");
        ...
        async function saveSpotsToFirestore() {
            try {
                await db.collection("parking").doc("spots").set({
                    spots: spots,
        ...
        function renderGrid() {
            ...
            for(let i = 0; i < TOTAL_SPOTS; i++) {
                ...
                let metaContent = '';
                if (spots[i]) {
                    if (spots[i].plate) metaContent += `<div class="meta">${spots[i].plate}</div>`;
                    if (spots[i].name) metaContent += `<div class="meta">${spots[i].name}</div>`;
                }
